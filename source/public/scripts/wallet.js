...
class Wallet {
    constructor(){
		this.MAINURL   = 'https://eth.bd.evmos.org:8545';
		this.TESTURL   = 'https://eth.bd.evmos.dev:8545';
		this.TESTEXP   = 'https://evm.evmos.dev';
		this.MAINEXP   = 'https://evm.evmos.org';
		this.wallet    = null;
		this.accounts  = [];
		this.myaccount = null;
		this.chainId   = null;
	...
	async start(enable=true){
		if(window.ethereum && window.ethereum.isMetaMask){
			...
			if(cid==9000 || cid==9001){
				// EVMOS OK
			} else {
				this.chainId = cid;
				//this.wallet = null;
				//web3 = null;
				console.log('Metamask not connected to EVMOS');
				if(enable){ alert('Metamask not connected to EVMOS'); }
			}
		} else {
